...
class TemplateDispatcher {
   public:
    ...
    TemplateDispatcher(TemplateDispatcher&& rhs) noexcept
        : q_(rhs.q_), prev_(rhs.prev_), f_(std::move(rhs.f_)), chained_(rhs.chained_) {
        rhs.chained_ = true;
    }

    TemplateDispatcher(MessageQueue* q, PreviousDispatcher* prev, F&& f) : q_(q), prev_(prev), f_(std::forward<F>(f)) {
        prev->chained_ = true;
    }
...
class Dispatcher {
   public:
    ...
    Dispatcher(Dispatcher&& rhs) noexcept : q_(rhs.q_), chained_(rhs.chained_) { rhs.chained_ = true; }
...
struct CancelWithdrawal {
    CancelWithdrawal(const std::string& _account, unsigned _amount) : account(_account), amount(_amount) {}
...
struct WithdrawalProcessed {
    WithdrawalProcessed(const std::string& _account, unsigned _amount) : account(_account), amount(_amount) {}
...
struct VerifyPIN {
    VerifyPIN(const std::string& _account, const std::string& pin_, Messaging::Sender _atm_queue)
...
struct GetBalance {
    GetBalance(const std::string& _account, Messaging::Sender _atm_queue) : account(_account), atm_queue(_atm_queue) {}
   ...
   private:
    ...
    void process_balance() {
        incoming_.wait()
            ...
            .handle<CancelPressed>([&](const CancelPressed& msg) { state_ = &ATM::done_processing; });
    }
    ...
    void verifying_pin() {
        incoming_.wait()
            .handle<PINVerified>([&](const PINVerified& msg) { state_ = &ATM::wait_for_action; })
        ...
        try {
            while (true) {
                incoming_.wait()
                    ...
                    .handle<GetBalance>([&](const GetBalance& msg) { msg.atm_queue.send(::Balance(balance_)); })
        ...
        try {
            while (true) {
                incoming_.wait()
                    ...
                    .handle<DisplayWithdrawalOptions>([&](const DisplayWithdrawalOptions& msg) {
                        {
                            std::lock_guard<std::mutex> l(m_);
                    ...
                    .handle<DisplayWithdrawalCancelled>([&](const DisplayWithdrawalCancelled& msg) {
                        {
                            std::lock_guard<std::mutex> l(m_);
                            std::cout << "Withdrawal cancelled" << std::endl;
                        }
                    })
                    .handle<DisplayPINIncorrectMessage>([&](const DisplayPINIncorrectMessage& msg) {
                        {
                            std::lock_guard<std::mutex> l(m_);
